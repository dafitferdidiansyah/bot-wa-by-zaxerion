...
const menit = 3 // durasi mute dalam menit
const SPAM_TOTALSPAM = 5; // Total pesan cepat sebelum dianggap spam
const SPAM_RESET_TIME = 30000; // reset counter spam dalam ms
const MAX_MESSAGE_DELAY = 2000; // max jeda antar pesan dalam ms
const SPAM_BAN_DURATION = 60000 * menit
const spam = true
const gcspam = false
const list = [".", "&", ",", "#", "$"] // karakter wajib di awal pesan
...
export async function before(m, { isPrems }) {
    ...
    if (!list.some(v => m.text && m.text.startsWith(v))) return !1
