...
let handler = async (m, { conn, text, participants, groupMetadata, isBotAdmin }) => {
	...
	try {
		let pp = 'https://raw.githubusercontent.com/dayoyui/dbs/refs/heads/main/media/avatar_contact.jpg'
		...
		let listAdmin = groupAdmins.map((v, i) => `${i + 1}. @${v.phoneNumber.split('@')[0]}`).join('\n')
...
_last group name changed by ${x.subjectOwnerPn ? '@' + x.subjectOwnerPn.split('@')[0] : 'Unknown'} on ${new Date(x.subjectTime * 1000).toDateString()}_

*Description :*
${x.desc ? x.desc.toString().replace(/chat.whatsapp.com\/(?:invite\/)?([0-9A-Za-z]{20,24})/g, '#HIDDENbyDefault') : 'None'}

*Group Owner :*
${x.ownerPn ? '@' + x.ownerPn.split('@')[0] : 'Unknown'}
